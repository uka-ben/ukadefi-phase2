...
def play_alert_sound(alert_type="neutral"):
    sound_url = {
        ...
        "bearish": "https://github.com/uka-ben/Sounder/raw/c2b3f81a8b704924eeb69efa668f805576740e34/alert2.wav",
        "neutral": "https://github.com/uka-ben/Sounder/raw/c2b3f81a8b704924eeb69efa668f805576740e34/alert3.wav"
...
with col1:
    symbol = st.selectbox("Symbol", ["ETH","BTC","SOL", "TRX", "ADA", "BCH", "PEOPLE", "IOTX", "POL","ATOM", "C98", "AAVE","DOT","MANA","CAKE", "XRP","ALICE", "BNB", "EOS", "ONE","ENJ","NEAR","VTHO","TRUMP","CRO","TLM","LUNC","ETC","STX","MBOX","SAND","UNI","DYDX","RUNE","DENT", "SHIB", "DOGE", "HOT", "CELR", "VET", "XLM", "ALGO", "GBP", "AUD","XAU", "EUR",])
with col2:
...
with st.sidebar:
    st.title("Controls")
    live_update = st.checkbox("Live Update", True)
    ...
    alert_sound = st.checkbox("Play Alert Sound", True)
...
def create_plot(data, price_line_colors, rsi_10_colors, symbol, interval, market_type):
    ...
    gs = fig.add_gridspec(num_plots, 1, height_ratios=[17] + [5] * len(visible_indicators), left=0.1, right=0.9)  # Added left/right margins
    ...
    for ax in axes:
        ...
        ax.tick_params(axis='both', which='major', labelsize=55, width=4, length=8, pad=8)
        ...
        ax.spins['top'].set_visible(False)
    ...
    for i in range(1, len(data)):
        axes[0].plot(data.index[i-1:i+1], data['Close'].iloc[i-1:i+1], color=price_line_colors[i], linewidth=5.0, alpha=1.0, solid_capstyle='round')

    # Divergence markers
    axes[0].scatter(data.index[data['Confirmed_Divergence'] == 1], data['Close'][data['Confirmed_Divergence'] == 1], color='green', marker='^', s=1000, edgecolor='black', linewidth=8, label='Bullish Div')
    axes[0].scatter(data.index[data['Confirmed_Divergence'] == -1], data['Close'][data['Confirmed_Divergence'] == -1], color='red', marker='v', s=1000, edgecolor='black', linewidth=8, label='Bearish Div')

    # Current price line
    axes[0].axhline(y=current_price, color='blue', linestyle='--', alpha=0.7, linewidth=5.0)
    axes[0].text(0.09, 1.10, f'{current_price:.5f} ({current_time})', transform=axes[0].transAxes, color='blue', fontsize=60, fontweight='bold', va='top', bbox=dict(facecolor='white', alpha=0.8, edgecolor='blue', linewidth=2, pad=5))
    axes[0].set_title(f'{market_type}: {symbol} ({interval})', fontsize=70, fontweight='bold', pad=20)
    axes[0].grid(True, linestyle='-', alpha=0.3, linewidth=5.0)

    # RSI plot
    plot_idx = 1
    for i in range(1, len(data)):
        axes[plot_idx].plot(data.index[i-1:i+1], data['RSI_10'].iloc[i-1:i+1], color=rsi_10_colors[i], linewidth=8.0, alpha=1.0, solid_capstyle='round', label='RSI 10' if i == 1 else "")
    axes[plot_idx].plot(data.index, data['RSI_22'], color='orange', linewidth=8.0, alpha=1.0, label='RSI 22')
    ...
    axes[plot_idx].text(0.01, 0.72, 'low void', transform=axes[plot_idx].transAxes, color='red', fontsize=45, fontweight='bold', va='center', bbox=dict(facecolor='white', alpha=0.8, edgecolor='red', linewidth=2))
    axes[plot_idx].text(0.01, 0.32, 'high void', transform=axes[plot_idx].transAxes, color='green', fontsize 45, fontweight='bold', va='center', bbox=dict(facecolor='white', alpha=0.8, edgecolor='green', linewidth=2)

    current_rsi = data['RSI_10'].iloc[-1]
    axes[plot_idx].axhline(y=current_rsi, color='blue', linestyle='--', alpha=1.0, linewidth=10.0)
    axes[plot_idx].annotate(f'{current_rsi:.2f}', xy=(0.98, current_rsi), xycoords='axes fraction', xytext=(-10, 0), textcoords='offset points', color='blue', fontsize=50, fontweight='bold', va='center', ha='right', bbox=dict(facecolor='white', alpha=0.8, edgecolor='black', linewidth=5, pad=4)
    axes[plot_idx].set_title('STRENGTH INDICATOR', fontsize=44, fontweight='bold', pad=15)
    plot_idx += 1

    # Fisher plot
    axes[plot_idx].plot(data.index, data['Ehlers_Fisher_Smoothed'], color='darkorange', linewidth=6.5, alpha=1.0, solid_capstyle='round', label='Fisher Transform')
    ...
    axes[plot_idx].set_title('FISHER TRANSFORMATION', fontsize=55, fontweight 'bold', pad=15)
    plot_idx += 1
    ...
    axes[plot_idx].axhline(0, color='black', linestyle='-', alpha=1.0, linewidth=3.0)
    axes[plot_idx].fill_between(data.index, data['Bias_Smoothed'], 0.2, where=data['Bias_Smoothed'] >= 0.2, facecolor='green', alpha=1.0, interpolate=True)
    axes[plot_idx].fill_between(data.index, data['Bias_Smoothed'], -0.2, where=data['Bias_Smoothed'] <= -0.2, facecolor='red', alpha=1.0, interpolate=True)
    axes[plot_idx].set_title('MARKET BIAS INDICATOR', fontsize=55, fontweight 'bold', pad=15)
    plot_idx += 1
    ...
    axes[plot_idx].scatter(data.index[strong_bullish], data['Divergence_Score'][strong_bullish], color='lime', marker='o', s=200, edgecolor='black', linewidth=3.5, label='Strong Bullish')
    axes[plot_idx].scatter(data.index[strong_bearish], data['Divergence_Score'][strong_bearish], color='darkred', marker='o', s=200, edgecolor='black', linewidth=3.5, label='Strong Bearish')
    axes[plot_idx].set_title('ANTI-SYMMETRIC BIDIVERGENCE SCORE', fontsize=55, fontweight 'bold', pad=15)
...
def swing_3(data):
    ...
    data['Divergence_Score'] = (
        data['DIV_RSI'].clip(-1, 1) + data['DIV_MACD'].clip(-1, 1) + data['DIV_KDJ'].clip(-1, 1) + data['DIV_CCI'])
    data["Potential_Divergence"] = (
        ((data['DIV_RSI'] + data['DIV_MACD'] + data['DIV_KDJ']) >= 2).astype(int) - ((data['DIV_RSI'] + data['DIV_MACD'] + data['DIV_KDJ']) <= -2).astype(int))
