...
def update_data_with_sse(prev_data, new_data_point):
    ...
    new_row = pd.DataFrame([{
        'Date': pd.to_datetime(new_data_point['timestamp']).tz_localize('UTC'),
...
def main_display():
    ...
    st.caption(f"Last update (UTC): {utc_now.strftime('%Y-%m-%d %H:%M:%S')} | {len(data)} bars loaded")

    with st.sidebar.expander("Recent Alerts", expanded=True):
        for alert in reversed(st.session_state.alert_history[-10:]):
            cols = st.columns([1, 2, 1])
            
            # Convert alert timestamp to UTC if it's not already
            ...
            cols[0].write(alert_time.strftime('%H:%M UTC'))
